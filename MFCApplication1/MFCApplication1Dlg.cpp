	...
	if (h_file_list != INVALID_HANDLE_VALUE) {
		...
		do {
			if (!(file_data.cFileName[0] == '.' && file_data.cFileName[1] == 0)) {// *.* 티렉토리 제외하기
				...
				memcpy(p->p_name, file_data.cFileName, (p->name_len + 1) << 1 /* `*2효과` */);
...
void CMFCApplication1Dlg::ChangeDir(CListBox * ap_list_box, int a_path_ctrl_id)
{
	...
	if (p->attr & FILE_ATTRIBUTE_DIRECTORY) {		// 디렉터리
		GetDlgItemText(a_path_ctrl_id, path);

		if (p->name_len == 2 && !memcmp(p->p_name, L"..", 4)) {		// 4bytes를 비교하는 이유는 유니코드라서 한 문자당 2바이트
			path.TrimRight('\\');
...
void CMFCApplication1Dlg::OnBnClickedRDleBtn()
{
	...
	if (index != LB_ERR) {
		...
		if(p->attr & FILE_ATTRIBUTE_DIRECTORY) {
			...
			if (IDOK == MessageBox(p->p_name, L"아래의 파일을 삭제하시겠습니까?", MB_ICONQUESTION | MB_OKCANCEL))
			{
				CString path;
				GetDlgItemText(IDC_R_PATH_EDIT, path);
